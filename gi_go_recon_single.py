#!/usr/bin/env python3
"""
GI/GO Reconciliation — Single-File CLI (no external files needed)

Features
- Reads Atlantis from file (API placeholder included for your future work)
- Reads GMI from FILE or ODBC (DB2/AS400)
- Uses **embedded SQL by default** (override with --sql-source file --sql-file path.sql)
- Generates GI and/or GO Excel reports (Top Summary by CB + detail buckets)
- Outputs to --outdir (defaults to P:\RECON)

Dependencies
    pip install pandas openpyxl pyodbc python-dotenv

Security
- Do NOT hardcode passwords. Pass with --odbc-pwd OR set env var GMI_PWD.
- This script will read .env automatically if present (using python-dotenv).

Examples
  # GMI via ODBC, embedded SQL, write to P:\RECON
  set GMI_PWD=YourPassword
  python gi_go_recon_single.py --atlantis-source file --atlantis-file "C:\data\Atlantis_Jul.csv" ^
    --gmi-source odbc --odbc-driver "Client Access ODBC Driver (32-bit)" ^
    --odbc-system 168.162.3.2 --odbc-uid PGHJH --report BOTH --month 2025-07 --log-level INFO

  # Both sides from local files
  python gi_go_recon_single.py --atlantis-source file --atlantis-file "C:\data\Atlantis_Jul.csv" ^
    --gmi-source file --gmi-file "C:\data\GMI_Jul.xlsx" --report BOTH --month 2025-07

  # Inspect the embedded SQL
  python gi_go_recon_single.py --print-sql
"""
import argparse
import os
import sys
...
def load_atlantis(atlantis_source: str, atlantis_file: Optional[str]) -> pd.DataFrame:
    if atlantis_source == "file":
        ...
        LOG.info("Loaded Atlantis file: %s (%d rows)", atlantis_file, len(df))
        return df
...
def fetch_gmi_via_odbc(driver: str, system: str, uid: str, pwd: str, sql_text: str) -> pd.DataFrame:
    ...
    try:
        ...
        cnxn.close()
    LOG.info("Fetched %d rows from GMI via ODBC", len(df))
    return df
...
def load_gmi(gmi_source: str, gmi_file: Optional[str],
    ...
    if gmi_source == "file":
        ...
        LOG.info("Loaded GMI file: %s (%d rows)", gmi_file, len(df))
        return df
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="GI/GO reconciliation (single-file)")
    # Atlantis
    ...
    p.add_argument("--month", help="Month filter YYYY-MM. If omitted, auto-detect latest.")
...
def run_one_mode(atlantis_df: pd.DataFrame, gmi_df: pd.DataFrame, mode: str, month: Optional[str], outdir: str) -> str:
    LOG.info("=== Running %s ===", "Give-Ins (GI)" if mode=="GI" else "Give-Outs (GO)")
    f1, f2 = prep_frames(atlantis_df, gmi_df, mode)
    f1, f2, month_used = month_detect_or_filter(f1, f2, month)
    LOG.info("Rows after filters — Atlantis: %d | GMI: %d | Month: %s", len(f1), len(f2), month_used)
...
def main(argv=None) -> int:
    ...
    try:
        if args.report in ("GI", "BOTH"):
            outputs.append(run_one_mode(atlantis_df, gmi_df, "GI", args.month, args.outdir))
        if args.report in ("GO", "BOTH"):
            outputs.append(run_one_mode(atlantis_df, gmi_df, "GO", args.month, args.outdir))
    except Exception as e:
    ...
    LOG.info("All done. Created: %s", ", ".join(outputs))
    return 0
